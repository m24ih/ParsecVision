...
function App() {
  ...
  const [analysisResult, setAnalysisResult] = useState(() => {
    const saved = localStorage.getItem("parsecVisionAnalysis");
    return saved ? JSON.parse(saved) : null;
  });
  const [cosmicExplanation, setCosmicExplanation] = useState(() => {
    return localStorage.getItem("parsecVisionExplanation") || null;
  });
  ...
  // Analiz verileri her değiştiğinde Local Storage'a kaydet (Ekran kapanırsa veriler silinmesin)
  useEffect(() => {
    if (analysisResult) {
      localStorage.setItem("parsecVisionAnalysis", JSON.stringify(analysisResult));
    } else {
  ...
  useEffect(() => {
    if (cosmicExplanation) {
      localStorage.setItem("parsecVisionExplanation", cosmicExplanation);
    } else {
      localStorage.removeItem("parsecVisionExplanation");
    }
  }, [cosmicExplanation]);

  // Sayfa manuel yenilendiğinde (F5) verileri temizle (Sıfırdan başlasın)
  useEffect(() => {
  ...
  const loadHistoryItem = (item) => {
    ...
    setAnalysisResult({
      ...
      astrometry: { ra: item.ra, dec: item.dec }
    });
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      if (analysisResult?.image_id) {
        const imgUrl = `${API_URL}/images/${analysisResult.image_id}.jpg`;
    ...
    <div className="container">
      <header className="top-header">
        ...
        <div style={{ color: "var(--highlight)", fontSize: "1.1rem", letterSpacing: "2px", fontWeight: "300" }}>
          {analysisResult?.astrometry 
            ? `TARGET: ${analysisResult.target_name || "DEEP SPACE"} | RA: ${analysisResult.astrometry.ra.toFixed(2)}°`
      ...
      <div className="sidebar">
        ...
        {analysisResult?.astrometry && (
          ...
          >
            ...
            <p>
              RA: {analysisResult.astrometry.ra.toFixed(4)}° | DEC:{" "}
              {analysisResult.astrometry.dec.toFixed(4)}°
            </p>
            <button
      ...
      <div className="map-area" id="export-map-area">
        {analysisResult ? (
          ...
          >
            <ImageOverlay
              url={`${API_URL}/images/${analysisResult.image_id}.jpg`}
              bounds={[
                [0, 0],
            ...
            <Rectangle
              ...
              eventHandlers={{
                ...
                click: () => {
                  ...
                  setSelectedDetail({
                    coordinates: `${analysisResult.astrometry.ra.toFixed(4)}°, ${analysisResult.astrometry.dec.toFixed(4)}°`,
      ...
      {/* YENİ SAĞ PANEL (AI INSIGHTS) - SADECE RESİM YÜKLENDİKTEN SONRA GÖRÜNÜR */}
      {analysisResult && (
        <div className="right-panel">
          <h3 style={{ color: "var(--highlight)", marginTop: 0, marginBottom: "20px", letterSpacing: "1px" }}>AI INSIGHT</h3>
          {!cosmicExplanation ? (
            <p style={{ color: "#777", fontStyle: "italic", textAlign: "center", marginTop: "50px" }}>Run 'ANALYZE REGION' to generate AI insights.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
              <div className="cosmic-explanation" style={{ flexGrow: 1, overflowY: "auto", paddingRight: "10px" }}>
                {formatAnalysis(cosmicExplanation)}
              </div>
              <button 
                onClick={exportToPDF} 
              ...
              historyData.length === 0 ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px", maxHeight: "50vh", overflowY: "auto", paddingRight: "10px" }}>
                  {historyData.map(item => (
                    ...
                    >
                      <h4 style={{ margin: "0 0 10px 0", color: "#fff" }}>{item.target_name}</h4>
